...
const mockScanFiles = mock(async function* (): AsyncGenerator<string> {});
...
describe('detectChanges', () => {
  ...
  it('should put new file in changed[] when it does not exist in DB', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'src/index.ts'; });
    spyOn(Bun, 'file').mockReturnValue(makeBunFile({ size: 100, lastModified: 1000 }));
    const fileRepo = makeFileRepo(new Map());

    const result = await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });
  ...
  it('should put file in deleted[] when it exists in DB but not on disk', async () => {
    mockScanFiles.mockImplementation(async function* () {});
  ...
  it('should compute contentHash when files are newly discovered', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'src/new.ts'; });
  ...
  it('should not call hashString when mtime and size are unchanged', async () => {
    ...
    await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });

    expect(mockHashString).not.toHaveBeenCalled();
  });

  it('should exclude files when extensions do not match', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'src/styles.css'; });
    const fileRepo = makeFileRepo(new Map());
  ...
  it('should exclude files when paths match ignorePatterns', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'node_modules/lib/index.ts'; });
    const fileRepo = makeFileRepo(new Map());
  ...
  it('should propagate error when Bun.file().text() throws', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'src/bad.ts'; });
    ...
    await expect(
      detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any })
  ...
  it('should return same result on second call when no files changed', async () => {
    ...
    const r1 = await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });
    const r2 = await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });
  ...
  describe('when new, changed, unchanged, and deleted files appear simultaneously', () => {
    ...
    beforeEach(async () => {
      ...
      result = await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: IGNORE_PATTERNS, fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });
    });
  ...
  it('should exclude top-level node_modules path from results', async () => {
    ...
    const result = await detectChanges({ projectRoot: PROJECT_ROOT, extensions: EXTENSIONS, ignorePatterns: [], fileRepo: fileRepo as any, scanFilesFn: mockScanFiles as any });
  ...
  it('should not exclude file whose name contains node_modules as substring', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'src/node_modules_helper.ts'; });
    spyOn(Bun, 'file').mockReturnValue(makeBunFile({ size: 50, lastModified: 500 }));
    const fileRepo = makeFileRepo(new Map());
  ...
  it('should exclude node_modules even when ignorePatterns is empty', async () => {
    mockScanFiles.mockImplementation(async function* () { yield 'node_modules/pkg/index.ts'; });
    spyOn(Bun, 'file').mockReturnValue(makeBunFile({ size: 50, lastModified: 500 }));
    const fileRepo = makeFileRepo(new Map());
