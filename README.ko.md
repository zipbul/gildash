...
| `plugins` | `LanguagePlugin[]` | — | 프레임워크 파일용 언어 플러그인 (예: Vue SFC용 `createVuePlugin()`, Svelte용 `createSveltePlugin()`). 플러그인 파일은 raw 좌표로 인덱싱되고 가상 TS 모듈로 시맨틱 질의에 답함; 해당 확장자를 `extensions`에 추가 |
