...
| `plugins` | `LanguagePlugin[]` | — | Language plugins for framework files (e.g. `createVuePlugin()` for Vue SFCs, `createSveltePlugin()` for Svelte). Plugin files are indexed at raw coordinates and answer semantic queries via virtual TS modules; add their extensions to `extensions` |
